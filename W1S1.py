...
Synced with the desktop copies, now using this as the main folder instead of the one on Desktop. Oof, fixing that was a mess.

Anyways, this should be the last update, and it'll just be knocking out the problems for this session as well as reformatting everything to match later problem sets. Namely, these things will be adjusted:
    - Tabs are now 4 spaces.
    - Docstrings are now "double quoted" and are placed inside functions, as is best practice. This header is excluded.
    - This header is present in every file, and updated accordingly with dates for start, finish, validation, and last update, plus notes.
    - The "UPI" or "UMPIRE" method is followed more, at least the "U" and "P". They are labeled in every function's docstring.
    - Tests are now directly below the function they're testing.
    - Subject to change, but there is now no main block.
